...
void Renderer::PushMatrix() const { glPushMatrix(); }
void Renderer::PopMatrix() const { glPopMatrix(); }
void Renderer::TranslateMatrix(double x, double y, double z) const { glTranslated(x, y, z); }
void Renderer::RotateMatrix(double angle) const { glRotated(angle, 0, 0 , -1); }
void Renderer::SetImagePixels(uint8* buffer, uint16 width, uint16 height) const {
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, buffer);
}
